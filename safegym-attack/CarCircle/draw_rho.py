...
ax.yaxis.set_ticks(np.arange(-2.5, 0.2, 0.5))
...
for i in range(0,4):
    m = i*0.5 - 1.5
    p = plt.plot( np.arange(0, len(pertub_rho_ave[i][0:200]))*0.05 , ysmoothed[i][0:200] +0.005 + m, linewidth=3.5,
               linestyle='dotted')

    plt.plot( np.arange(0, len(pertub_rho_ave[i][0:200]))*0.05, rho_ave[i][0:200]+ m, linewidth=3, color = p[0].get_color(), label=label[i])
...
plt.savefig('rho_carcircle.pdf', bbox_inches='tight', dpi=500)
