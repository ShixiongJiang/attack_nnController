...
from safetygym_benchmark.PointGoal.attack_methods import black, white, grey_c, grey_s
...
for epsilon in [0.10]:
    ...
    _, _, grey_c_dist_list = grey_c(env=env, model=model, surro_model=surro_model, adv_model=adv_model, epsilon=epsilon,
                              total_epoch=total_epoch)
    _, _, grey_s_dist_list = grey_s(env=env, model=model, surro_model=surro_model, adv_model=adv_model, epsilon=epsilon,
                              total_epoch=total_epoch)
    ...
    laa(env=env, model=model, surro_model=surro_model, adv_model=adv_model, laa_model=laa_model, epsilon=epsilon,
        total_epoch=total_epoch)
    print('++++++++++++')
...
plt.savefig('pointgoal.pdf', bbox_inches='tight', dpi=500)
