...
from safetygym_benchmark.CarCircle.attack_methods import black, white, grey_c, grey_s
from utils.baseline_attack import gradient, mad, laa
env = CarCircleEnv()
model = PPO.load('./model/PPO_carcircle.zip')
surro_model = SAC.load('./model/surro_SAC_carcircle.zip')
adv_model = PPO.load('./model/adv_PPO_carcircle.zip')
laa_model = PPO.load('./model/laa_PPO_carcircle.zip')
...
for epsilon in [0.10]:
    print(f'Carcircle Benchmark with epsilon {epsilon}')
    _, _, black_dist_list = black(env=env,  model=model, surro_model=surro_model, adv_model=adv_model, epsilon=epsilon, total_epoch= total_epoch)
    ...
    _, _, grey_c_dist_list = grey_c(env=env,  model=model, surro_model=surro_model, adv_model=adv_model, epsilon=epsilon , total_epoch= total_epoch)
    _, _, grey_s_dist_list = grey_s(env=env,  model=model, surro_model=surro_model, adv_model=adv_model, epsilon=epsilon, total_epoch= total_epoch)
